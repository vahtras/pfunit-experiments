...
contains
    ...
    use infhso_mod, only: one_electron_spin_orbit_labels, &
        mean_field_spin_orbit_labels, scaled_spin_orbit_labels
    use list_queries, only: is_in_list
    implicit none
    character*8 label
    logical is_one
    is_one = is_in_list(label, one_electron_spin_orbit_labels).or. &
        is_in_list(label, mean_field_spin_orbit_labels).or. &
        is_in_list(label, scaled_spin_orbit_labels)
    ...
    function is_full_spin_orbit(label) result(is_valid)
    use infhso_mod, only: full_electron_spin_orbit_labels
    use list_queries, only: is_in_list
    character*8 label
    logical is_valid
    is_valid = is_in_list(label, full_electron_spin_orbit_labels)
    end function is_full_spin_orbit
